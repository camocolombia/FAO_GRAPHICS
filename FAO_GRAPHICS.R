  require(ggplot2);library(gridExtra)
  
  source("E:/Dropbox/Dropbox/Paper_PhD/FAO/multiplot_function.R")
  ...
  p1 <- ggplot(FAO_data, aes(x=Year, y=Value,colour=Region, group = Region)) +
  ...
  #  ylab("Gigagrams of CH4") +
    ...
    scale_y_continuous(breaks=seq(400,9000,300),limits = c(400,9000))+
  ...
  p1  <-  p1 + annotate("text", x = 1961, y = 9000, label = "A", size = 40)
  
  
  ggsave(paste0("E:/Dropbox/Dropbox/Paper_PhD/FAO","/","LINE_EMMISIONS_SHEEP",".pdf"),p1,dpi=600,width =90,height=60,units = "cm",scale=1.2,limitsize = FALSE)
  ...
  p2  <-  p2 + annotate("text", x = 1961, y = 1300, label = "B", size = 40)
  ...
  #p3  <-  p3 + annotate("text", x = 0, y = 1, label = "C", size = 40)
  ...
  p4  <-  p4 + annotate("text", x = 1961, y = 1700, label = "C", size = 40)
  ...
  p5  <-  p5 + annotate("text", x = 1961, y = 1700, label = "D", size = 40)
  ...
  test <- ggplot(FAO_data, aes(x=Year, y=Value,colour=Region, group = Region)) + 
     geom_line(size=8)+
   scale_colour_brewer(type = "qual", palette = "Set1")+
    theme(legend.position="bottom" ,
          ...
          )#,fill = c("#DDE6DA","#F1E9D9","E9F1D9","F3E2D9")
   test <- test + guides(shape=guide_legend(override.aes=list(size=60)),legend.title=element_blank(),title=NULL)
  
   test  <-  test + guides(colour = guide_legend(nrow = 1))

  mylegend <- g_legend(test)
  #####################################################################################
  #####################################################################################
  ...
  pp  <-  grid.arrange(arrangeGrob(p1 + theme(legend.position="none"),
                                 ...
                                 nrow=2),
                     mylegend, nrow=2,heights=c(40,4))
  
  ggsave(paste0("E:/Dropbox/Dropbox/Paper_PhD/FAO","/","LINE_JOINT",".pdf"),pp,dpi=400,width =190,height=120,units = "cm",scale=1.2,limitsize = FALSE)
